...
import { getPurchases, formatCurrency } from "@/lib/erpnext";
...
import { Search, Receipt, ArrowUpDown, Filter, MoreVertical, Clock, CheckCircle, AlertCircle, Truck } from "lucide-react";
...
const PurchaseOrderDetails: React.FC<PurchaseOrderDetailsProps> = ({ isOpen, onClose, order }) => {
  if (!order) return null;
  
  return (
    <BottomSheet open={isOpen} onOpenChange={onClose}>
      <BottomSheetHeader title={`Purchase #${order.name}`}>
        <Button 
          variant="ghost" 
          size="icon" 
      ...
      <BottomSheetBody>
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <Badge 
              variant={order.status === "To Receive and Bill" ? "outline" : "default"}
              className={
                order.status === "Completed" ? "bg-green-100 text-green-800 hover:bg-green-100" : 
                order.status === "To Receive and Bill" ? "bg-amber-100 text-amber-800 hover:bg-amber-100" :
...
const Purchases: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/purchases', MOCK_USER_ID],
    queryFn: () => getPurchases(MOCK_USER_ID),
  });
  ...
  const filteredOrders = React.useMemo(() => {
    ...
    if (activeTab !== "all") {
      filteredByStatus = orders.filter((order: any) => {
        if (activeTab === "pending" && order.status === "To Receive and Bill") return true;
    ...
    return (
      <div className="p-4">
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg">
          <h3 className="font-medium">Failed to load purchase orders</h3>
  ...
  const statusCounts = purchasesData.reduce((acc: any, order: any) => {
    const status = order.status;
    if (status === "To Receive and Bill") acc.pending = (acc.pending || 0) + 1;
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center">
            <p className="text-gray-500">
              {searchQuery ? "No purchase orders match your search" : "No purchase orders available"}
            </p>
          </div>
        ) : (
          filteredOrders.map((order: any) => (
            ...
            >
              <div className="flex items-start">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center mr-3 flex-shrink-0 bg-purple-100">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Badge 
                      ...
                      className={
                        ...
                        order.status === "To Receive and Bill" ? "bg-amber-100 text-amber-800" :
